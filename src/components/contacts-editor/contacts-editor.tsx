...
import type { BsContactsEditorProps, ContactEditFormProps } from './contacts-editor.d'
...
function makeContactFieldChangeHandler(
  props: ContactEditFormProps,
  field: 'address' | 'email' | 'firstName' | 'lastName' | 'phone'
) {
  return (value: string) => {
...
function makeContactRelationshipChangeHandler(props: ContactEditFormProps) {
  return (value: string) => {
    if (isContactRelationship(value)) {
      props.onChange({ relationship: value })
    }
  }
}

function ContactEditForm(props: ContactEditFormProps) {
  const onLastNameChange = makeContactFieldChangeHandler(props, 'lastName')
  const onFirstNameChange = makeContactFieldChangeHandler(props, 'firstName')
  const onPhoneChange = makeContactFieldChangeHandler(props, 'phone')
  const onEmailChange = makeContactFieldChangeHandler(props, 'email')
  const onAddressChange = makeContactFieldChangeHandler(props, 'address')
  const onRelationshipChange = makeContactRelationshipChangeHandler(props)
  ...
  return (
    <div class="flex flex-col gap-2 rounded-lg bg-base-200 p-3">
      ...
      <div class="footer-buttons-container">
        ...
        <button class="btn btn-primary btn-wide" onClick={props.onSave} type="button">
          <Save />
          Enregistrer
        </button>
      </div>
