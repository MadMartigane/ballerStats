...
- [x] [P1] ~~Reduce `pnpm run check` errors (258 currently, breaks the ambient "check must pass" gate). Breakdown: 103 lint/style/useConsistentMemberAccessibility, 25 lint/style/noExportedImports + 23 lint/performance/noBarrelFile (index.ts barrels), ~40 useConsistentArrayType/useConsistentTypeDefinitions/useOptionalChain/useReadonlyClassProperties (mostly auto-fixable via `pnpm run fix`), 7 lint/suspicious/useAwait, 2 unused biome-ignore suppressions in src/components/combobox/combobox.tsx, a11y one-offs. Suggest incremental PRs per rule family, starting with auto-fixable ones.~~ — `pnpm run check` now passes at 0 errors / 0 warnings / 0 infos (lint to zero, barrels removed, typecheck to zero, package upgrades + husky pre-commit hook)
